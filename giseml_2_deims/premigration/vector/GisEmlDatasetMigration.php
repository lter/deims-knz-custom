...
class GisEmlDatasetMigration extends XMLMigration {
  ...
  public function __construct(array $arguments) {
    ...
    $this->description = t('GIS Vector EML migration of dataset metadata');
   ...
   //    'associatedparties' => t('The dataset associated roles'),
        ...
        'revisionid' => t('The dataset GIS Vector EML revision id'),
    );
...
//////////////////////////////////////////////////////
    ...
    drupal_get_path('module', 'giseml_2_deims') . '/xml/vector/';
...
//    ));
  ...
  public function prepareRow($row) {
    ...
    if(!isset($row->xml->dataset->alternateIdentifier)){
        $row->xml->dataset->alternateIdentifier = '';
    }
...
//      $row->xml->dataset->coverage->temporalCoverage->rangeOfDates->endDate->calendarDate = $mybdate;
   ...
   // $row->xml->dataset->spatialVector->entityName = $vectornameid;
    ...
    if(isset($row->xml->dataset->spatialVector->methods->methodStep->description->para)){
...
//        watchdog('GISEML2DEIMS:', "Ds-Person query yield no matches $strquery ");
  ...
  public function getVectorSource($row) {

    if (!empty($row->xml->dataset->spatialVector->entityName)) {
      $query = new EntityFieldQuery();
      $query->entityCondition('entity_type', 'node');
      $query->entityCondition('bundle', 'data_source');
      $query->propertyCondition('title', $row->xml->dataset->spatialVector->entityName, 'CONTAINS');
      $results = $query->execute();
      if (!empty($results['node'])) {
        $nid = reset($results['node'])->nid;
